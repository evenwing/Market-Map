...
export async function createTrace({ input, sessionId, parentSpanIds }) {
  ...
  const addMetadata = (patch) => {
    if (!patch || typeof patch !== "object") return;
    extraMetadata = mergeMetadata(extraMetadata, patch);
  };
  ...
  return {
    ...
    async end(output, renderHtml) {
      ...
      if (renderHtml !== null && renderHtml !== undefined) {
        metadata.render_html = renderHtml;
      }
      if (rootSpan) {
        endLlmSpan();
    ...
    getSpanIds() {
      ...
      if (!rootSpan.spanId || !rootSpan.rootSpanId) return null;
      return {
        span_id: rootSpan.spanId,
        root_span_id: rootSpan.rootSpanId
      };
    },
    startSpan,
    addMetadata
...
function mergeMetadata(base, patch) {
  ...
  Object.entries(patch).forEach(([key, value]) => {
    if (Array.isArray(value) && Array.isArray(next[key])) {
      next[key] = next[key].concat(value);
      return;
    }
