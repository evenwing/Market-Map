...
function buildPrompt(input, attempt, lastError) {
  const base = [
    "You are a market research analyst for software or technology market categories.",
    "Task: infer the closest software or technology market category for the user input, then list the top 3 players.",
    "Do not ask clarifying questions. Return mode=apology if the input is empty or unrelated to technology (eg: emoji-only, random characters).",
    "Use Google Search grounding when the user asks for fresh data (mentions 2025, 2026, etc.). Otherwise do not use tools.",
    "Ranking priority: revenue -> valuation -> number of customers -> number of G2 ratings above 4.",
    "Include ranking_justification as a short bullet list (plain text) that clearly explains why #1 > #2 > #3 using the metrics and ranking basis.",
    "Provide exactly 3 companies. Each company must include at least 2 metrics to support the ranking and a differentiated value_prop statement.",
    "Value_prop statement should explain succinctly without jargon why customers prefer this vendor uniquely over other options.",
    "Only include metrics backed by numeric evidence from filings, company websites, press releases, G2, Gartner, Forrester, etc.",
    "Return at least 2 valid citations per company used to support the metrics.",
    "Include a top-level thinking_summary with 4-6 sentences that summarize key choices at a high level (category, ranking basis, company selection, and how sources/citations were chosen).",
    "In thinking_summary, list the top 5 companies considered and explain briefly why #4 and #5 did not make the final top 3. Do not reveal chain-of-thought or step-by-step reasoning.",
    ...
    "  \"ranking_basis\": \"revenue\" | \"valuation\" | \"customers\" | \"g2_ratings_4plus\" | null,",
...
function buildPlanPrompt(input, attempt, lastError) {
  const base = [
    "You are a market research analyst for software and technology marketcategories.",
    "Task: identify the closest software or technology market category for the user input and draft a short research plan to identify the top 3 companies.",
    "Ranking priority: revenue -> valuation -> number of customers -> number of G2 ratings above 4.",
    "The plan must include: sources you plan to use, metrics you will gather to support the ranking, and your ranking approach.",
    "Ask one clarifying question with 1-2sub-bullets to refine the plan (region, segment, timeframe, customer size, etc.).",
    ...
    "  \"mode\": \"plan\",",
    "  \"category\": string,",
    "  \"ranking_basis\": \"revenue\" | \"valuation\" | \"customers\" | \"g2_ratings_4plus\",",
    "  \"plan\": {",
    "    \"sources\": [string],",
    "    \"metrics\": [string],",
    "    \"approach\": string",
...
function buildPlanExecutionPrompt(input, attempt, lastError, context) {
  ...
  const base = [
    ...
    "Return top 3 ranked companies with metrics, value prop, and citations.",
    "Each company must include at least 2 metrics to support the ranking and a differentiated value_prop statement.",
    "Value_prop statement should explain succinctly without jargon why customers prefer this vendor uniquely over other options.",
    ...
    "",
    "Plan context:",
    `Category: ${safeString(plan.category) || safeString(context?.category) || "Software/Technology Market"}`,
    `Ranking basis: ${safeString(plan.ranking_basis) || "revenue" | "valuation" | "customers" | "g2_ratings_4plus"}`,
    `Planned sources: ${Array.isArray(planDetails.sources) ? planDetails.sources.join(", ") : ""}`,
    `Planned metrics: ${Array.isArray(planDetails.metrics) ? planDetails.metrics.join(", ") : ""}`,
    `Planned approach: ${safeString(planDetails.approach) || ""}`,
...
function buildCitationRepairPrompt(input, attempt, lastError, context) {
  ...
  const base = [
    "You are fixing broken citations for a software and technology market report.",
...
function normalizeOutput(raw) {
  ...
  if (raw.mode === "apology") {
    return {
      ...
      apology: {
        ...
        message: safeString(raw.apology?.message) || "Sorry! I analyze software and technologymarkets.",
        hint: safeString(raw.apology?.hint) || "Try Again: CRM, payments, video conferencing."
      }
    };
  }
  ...
  return {
    ...
    ranking_justification: safeString(raw.ranking_justification),
...
function normalizePlanOutput(raw) {
  if (!raw || typeof raw !== "object") {
    return {
      mode: "plan",
      category: "Software/Technology Market",
  ...
  return {
    mode: "plan",
    category: safeString(raw.category) || "Software/Technology Market",
    ranking_basis: normalizeRankingBasis(raw.ranking_basis) || "revenue",
    plan: {
      sources,
      metrics,
...
function validateOutput(output) {
  ...
  if (!safeString(output.ranking_justification)) {
    errors.push("Missing ranking_justification");
  }
...
function buildDefaultApology() {
  return {
    title: "Signal Lost",
    message: "Sorry - I analyze software and technology markets.",
    hint: "Try Again: CRM, payments, video conferencing."
  };
}
