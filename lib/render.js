export function renderHtml(payload) {
  ...
  const category = escapeHtml(payload.category || "Software Market");
  const ranking = escapeHtml(formatRankingBasis(payload.ranking_basis));
  const justification = renderRankingJustification(payload.ranking_justification);
...
function renderRankingJustification(value) {
  const items = normalizeJustificationItems(value);
  if (!items.length) return "";
  const list = items.map((item) => `<li style="margin:0 0 6px;">${escapeHtml(item)}</li>`).join("");
  return `<div style="padding:0 32px 12px;">
    <ul style="margin:0;padding-left:18px;color:#cfcfcf;font-size:13px;line-height:1.5;">${list}</ul>
  </div>`;
}

function normalizeJustificationItems(value) {
