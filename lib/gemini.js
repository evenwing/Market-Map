...
export async function analyzeMarket(input, recordEvent, options = {}) {
  ...
  const useTools =
    typeof options.useTools === "boolean" ? options.useTools : shouldUseTools(input);
  return await analyzeWithRetry(input, recordEvent, 1, "", {
    model: preferredModel,
    triedFallback: false,
    useTools,
    triedToolsFallback: false,
    transientAttempt: 0,
...
function buildPrompt(input, attempt, lastError) {
  const base = [
    ...
    "Use Google Search grounding tool only when needed (vague input or missing data); otherwise answer without tools to minimize latency.",
